    ...
    while (1) {
        ...
        if ( true == data_ready ) {
            ...
            for (uint8_t channel = 0; channel < ADC_SEQUENCED_CHANNELS_NUM; channel++) {
                ...
                ADC_Data.volts[channel] = ADC_CountsTo_Volts(channel, (int16_t) ADC_Data.counts[channel]);
                snprintf(ADC_Data.string, string_size, "CH %d: %.3f Volts\r\n", channel, ADC_Data.volts[channel]);
                UART_PutString(ADC_Data.string);
            }
            
            // Fill the data we want to send to the rpi
            
            // Notify the rpi we got new data
            READY_Write(1);
